...
var Topics = []Topic{
	...
	},
	{
		...
		RequiredCapability: units.CapEditContent,
		Body: []string{
			"The homepage's text and photos are edited from \"Edit Homepage\" — the hero banner, a short \"why us\" intro, what your program offers, meeting details, and contact information. The \"why us\" box sits directly under the hero and starts empty on purpose: it is the one thing no default text can write for you, and until you write something there is no section on the page at all.",
			...
			"Once you've published a leader profile or two, the homepage's Leadership & Contact box shows the first three of them — name, role and the start of each bio — and links through to the full Our Leaders page for the rest. They are the first three in the order the leaders page itself uses — lowest \"Sort order\" first, then alphabetically — so giving someone a lower sort order on their profile moves them up in both places at once. Until you publish any, the box stays as the paragraph you typed, with no link, so nobody is sent to an empty page.",
