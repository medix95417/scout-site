...
**A note on scope of the "re-audit":** everything below is a code-level review of what's in this repository — the same method as the original pass. It is not a live penetration test of the running VPS (this environment has no network path to `troop.47-yonkers.org` / `pack.47-yonkers.org`). At the time of this first pass it also didn't replace a `docker compose up -d --build` confirming the code actually compiles and runs, since the sandbox running it that day couldn't fetch `golang.org/x/crypto` or run a real `go build`. **That stopped being true starting with pass 3** (2026-08-22) — every pass from there on, including the security-key work and the roster privilege-ceiling fix in pass 4, was run against a real, compiled, running server with a live Postgres and (for pass 4's live checks) a real browser; see each pass's own "Verification" section for exactly what it ran. See "Verification" below for what pass 1 specifically did and didn't check.
...
- A custom Go-AST-based checker (no external dependencies, since the sandbox running this first pass had no network access for `go mod`) swept the whole repo for unused imports after every change. It flags `github.com/jackc/pgx/v5` as an "unused import" in three files (`auth.go`, `approval.go`, `roster.go`) — this is a confirmed false positive specific to this checker: the import path ends in `v5` but the actual package name is `pgx`, and `grep "pgx\."` in each file confirms real usage (e.g. `pgx.ErrNoRows`, `pgx.Tx`). (This workaround was specific to that day's constraints — `go vet`, which every pass since has had available, catches real unused-import errors directly and doesn't need it.)
